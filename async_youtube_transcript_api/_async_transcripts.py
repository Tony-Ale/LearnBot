...
class TranscriptListFetcher(object):
    ...
    async def fetch(self, video_id):
        video_html = await self._fetch_video_html(video_id)
    ...
    async def _create_consent_cookie(self, html, video_id):
        match = re.search('name="v" value="(.*?)"', html)
        if match is None:
            raise FailedToCreateConsentCookie(video_id)
        cookie_jar = self._session.cookie_jar
        cookie_jar.update_cookies({'CONSENT': 'YES+' + match.group(1)}, URL('.youtube.com'))

    async def _fetch_video_html(self, video_id):
        html = await self._fetch_html(video_id)
        if 'action="https://consent.youtube.com/s"' in html:
            await self._create_consent_cookie(html, video_id)
            html = await self._fetch_html(video_id)
            if 'action="https://consent.youtube.com/s"' in html:
                raise FailedToCreateConsentCookie(video_id)
        return html

    async def _fetch_html(self, video_id):
        async with self._session.get(WATCH_URL.format(video_id=video_id), headers={'Accept-Language': 'en-US'}) as response:
            return unescape(await _raise_http_errors(response, video_id).text())
...
class TranscriptList(object):
    ...
    @staticmethod
    def build(session, video_id, captions_json):
        """
...
class Transcript(object):
    def __init__(self, session:aiohttp.ClientSession, video_id, url, language, language_code, is_generated, translation_languages):
        """
    ...
    async def fetch(self, preserve_formatting=False):
        """
        ...
        """
        async with self._session.get(self._url, headers={'Accept-Language': 'en-US'}) as response:
            return _TranscriptParser(preserve_formatting=preserve_formatting).parse(
                await _raise_http_errors(response, self.video_id).text(),
            )
