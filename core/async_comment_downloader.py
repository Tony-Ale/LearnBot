...
class AsyncYoutubeCommentDownloader:
    ...
    def __init__(self, num_workers=None, max_num_comments=500):
    ...
    async def _worker(self, url_queue:asyncio.Queue, response_queue:asyncio.Queue, *args, **kwargs):
        while True:
            ...
            try:
                # perform task 
                response = await self._get_comments_from_url(url, *args, **kwargs)
            ...
            response_queue.put_nowait({video_id:response})
    ...
    def get_comments(self, youtube_id_list:list|str, *args, **kwargs):
    ...
    def get_comments_from_url(self, urls:list|str, sort_by=SORT_BY_RECENT, language=None, sleep=.1):
        ...
        async with self.session.get(youtube_url) as response:
            ...
            while continuations:
                ...
                for comment in reversed(list(self.search_dict(response, 'commentEntityPayload'))):
                    ...
                    if len(output) == self.max_num_comments:
                        return output
                await asyncio.sleep(sleep)
            return output
